...
describe("CreatorTable", () => {
  ...
  it("shows every column the admin needs for one creator", () => {
    ...
    expect(
        within(row).getByText("Rangga Pratama"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("rangga@example.com"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("Instagram @rangga.creates"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("TikTok @ranggacreates"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("Kontrak aktif"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("10 Jun – 7 Des 2026"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("sisa 80 hari"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("5/6 konten terkirim"),
    ).toBeInTheDocument();
    ...
    expect(
        within(row).getByText("80%"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("0,17x"),
    ).toBeInTheDocument();

    expect(
        within(row).getByText("Baik"),
    ).toBeInTheDocument();
  });
  ...
  it("mentions the period once a contract has been renewed", () => {
    ...
    expect(
        screen.getByText("sisa 80 hari, periode ke-2"),
    ).toBeInTheDocument();
  });
  ...
  it("describes an expired, an upcoming and a missing contract", () => {
    ...
    expect(
        screen.getByText("Kontrak berakhir"),
    ).toBeInTheDocument();

    expect(
        screen.getByText("berakhir 19 hari lalu"),
    ).toBeInTheDocument();

    expect(
        screen.getByText("Kontrak belum mulai"),
    ).toBeInTheDocument();

    expect(
        screen.getByText("28 Sep 2026 – 27 Mar 2027"),
    ).toBeInTheDocument();

    expect(
        screen.getByText("Belum ada kontrak"),
    ).toBeInTheDocument();
  ...
  it("flags revoked access next to the identity", () => {
    ...
    expect(
        screen.getByText("Akses dicabut 4 Sep 2026"),
    ).toBeInTheDocument();
  });
  ...
  it("shows the other productivity bands with their own tone", () => {
    ...
    expect(
        screen.getByText("Belum Ada Data").className,
    ).toContain("amber");

    expect(
        screen.getByText("Berisiko").className,
    ).toContain("red");
  ...
  it("explains an empty roster", () => {
    ...
    expect(
        screen.getByText(
            "Belum ada creator yang terdaftar.",
        ),
    ).toBeInTheDocument();
  });
  ...
  it("points back to the first page when the page is past the end", () => {
    render(
        <CreatorTable creators={[]} total={12} />,
    );

    expect(
        screen.getByText(
            "Tidak ada creator di halaman ini.",
        ),
    ).toBeInTheDocument();
  ...
  it("shows Detail and Content Plan actions for each creator", () => {
    render(
        <CreatorTable creators={[creator()]} total={1} />,
    );
