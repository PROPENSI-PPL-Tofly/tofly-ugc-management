...
describe('CreatorsController', () => {
  ...
  const response = { items: [], page: 1, pageSize: 10, total: 0, totalPages: 1 };
  ...
  it('rejects a page below one', async () => {
    await expect(controller.list(0, 10)).rejects.toBeInstanceOf(BadRequestException);
    expect(service.list).not.toHaveBeenCalled();
  });

  it('rejects an empty page size', async () => {
    await expect(controller.list(1, 0)).rejects.toBeInstanceOf(BadRequestException);
  });

  it('caps the page size so one request cannot pull the whole roster', async () => {
    await expect(controller.list(1, 51)).rejects.toBeInstanceOf(BadRequestException);
